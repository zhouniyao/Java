...
import niming.util.MatrixStack;
...
//  int texture1;
    ...
    ObjParser obj;
    final Handler hd;
    int TYPE = Constant.FACES; //以什么形式绘制图形，默认是三角面绘制
...
//    	speed += -0.001;// 速度的阻尼系数
        ...
        if((endTime - startTime) > 1000){//每秒输出一次
        	DecimalFormat dFormat = new DecimalFormat("##0.000");//格式设置小数点后3位
        	String msg = dFormat.format(binocularDistance);//按照上述设置的 格式做给String对象
