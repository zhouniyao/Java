...
//		 obj = new ParseObj(context, R.raw.boot);
...
//				.put(new byte[]{
//						1-1,2-1,4-1,  1-1,4-1,3-1,
//						3-1,4-1,6-1,  3-1,6-1,5-1,
//						5-1,6-1,8-1,  5-1,8-1,7-1,
//						7-1,8-1,2-1,  7-1,2-1,1-1,
//						2-1,8-1,6-1,  2-1,6-1,4-1,
//						7-1,1-1,3-1,  7-1,3-1,5-1
//						
//				});
...
//		 Matrix.orthoM(projectionMatrix, 0, -1, 1, -(float)width/(float)height, (float)width/(float)height, 0f, 20f);//不对
		 Matrix.orthoM(projectionMatrix, 0, -(float)width/(float)height, (float)width/(float)height, -1, 1, 0f, 20f);
...
//		glDrawElements(GL_TRIANGLES, 6*6, GL_UNSIGNED_BYTE, indexArray);//提示数组越界    obj.vIndexNum
    ...
    private void drawUmbrella() {
        setIdentityM(modelMatrix, 0);
        translateM(modelMatrix, 0, 0f, 0f, -50f);//平移变换
        rotateM(modelMatrix, 0, - yRotate, 1f, 0f, 0f);
        rotateM(modelMatrix, 0, - xRotate, 0f, 1f, 0f);
        yRotate++;
        xRotate++;
//        setLookAtM(viewMatrix, 0, xRotation, yRotation, 2.2f, 0f, 0f, 0f, 0f, 1f, 0f);//立方体会放大、缩小
		...
		glUniform4f(uColorLocation, 1.0f, 1.0f, 1.0f, 1.0f);
		glDrawElements(GL_TRIANGLES, obj.vIndexNum, GL_UNSIGNED_BYTE, indexArray);//提示数组越界    obj.vIndexNum
		Log.v(TAG, "顶点索引总数：" + obj.vIndexNum);
    }
