...
public class ShaderHelper {
    ...
    /**
     * Compiles a shader, returning the OpenGL object ID.type's value：GL_VERTEX_SHADER or GL_FRAGMENT_SHADER. 
     */
    private static int compileShader(int type, String shaderCode) {
        
        // Create a new shader object.
        final int shaderObjectId = glCreateShader(type);//创建一个新的着色器对象，并把这个对象的ID存入shaderObjectId
        ...
        // Pass in the shader source. 上传源代码
        glShaderSource(shaderObjectId, shaderCode);//这个调用告诉OpenGL读入字符串shaderCode定义的源代码，并把它与shaderObjectId所引用的着色器对象关联起来。
    ...
    //这个辅助函数会编译vertexShaderSource和fragmentShaderSource定义的着色器，并把它们链接在一起成为一个程序。
